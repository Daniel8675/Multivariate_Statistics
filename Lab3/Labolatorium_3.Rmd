...
W trakcie zajęć wykorzystywany będzie zbiór danych `Smarket` z pakietu `ISLR`. Opisuje on zachowanie indeksu giełdowego S&P 500 przez kolejne dni od roku 2001 do roku 2005. Zmienna `Direction` jest czynnikiem o 2 poziomach podsumowującym kierunek zmiany indeksu w danym dniu, tzn. `Direction == "Up"` wtedy i tylko wtedy, gdy `Today >= 0`.
...
wskazuje na to, że zauważalna korelacja występuje tylko między zmiennymi `Year` i `Volume`. Wykres rozproszenia
...
Chcemy dopasować model regresji logistycznej żeby przewidzieć wartość `Direction` na podstawie zmiennych `Lag1`, ..., `Lag5` i `Volume`.
...
Do predykcji --- podobnie jak w regresji liniowej --- wykorzystywana jest funkcja `predict()`. Przy wartości parametru `type = "response"` funkcja zwraca przewidywane prawdopodobieństwa $P(Y=1|X)$ (a nie np. szanse logarytmiczne).
...
Do zobrazowania wyników klasyfikacji używamy *tablicy pomyłek* (*confusion matrix*)
...
```{r conf matrix 2}
dir_log_t$cm <- table(dir_log_t$predicted, Smarket_test$Direction)
dir_log_t$cm
```

```{r accuracy 2}
(dir_log_t$cm[1, 2] + dir_log_t$cm[2, 1]) / sum(dir_log_t$cm)
mean(dir_log_t$predicted != Smarket_test$Direction)
```

Próbujemy uzyskać bardziej efektywny model eliminując najmniej istotne predyktory --- wszystkie mają słabe lub bardzo słabe p-wartości, ale najlepsze dwa to `Lag1` i `Lag2`.
...
W sytuacji jak poprzednio stosujemy LDA do klasyfikacji wyznaczonej przez `Direction` względem `Lag1` i `Lag2`.
...
```{r accuracy 4}
mean(dir_lda$predicted$class != Direction_test)
```
...
```{r accuracy 5}
mean(dir_qda$predicted$class != Direction_test)
```
...
Ze względu na to, że kNN rozstrzyga remisy losowo, dla zapewnienia powtarzalności wyników warto przed wywołaniem funkcji `knn` zainicjalizować generator liczb pseudolosowych (`?set.seed`).
...
```{r}
dir_knn_3 <- knn(train_set, test_set, Direction_train, k = 3)
table(dir_knn_3, Direction_test)
```

```{r accuracy 7}
mean(dir_knn_3 != Direction_test)
```
